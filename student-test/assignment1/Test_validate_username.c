...
void test_validate_my_username()
{
    ...
    // Step 2: Get username from the configuration file (malloc’ed string)
    char* actual_username = malloc_username_from_conf_file();

    // Step 3: Verify both are equal
    TEST_ASSERT_EQUAL_STRING_MESSAGE(expected_username, actual_username, "Username in conf file does not match my_username()");
